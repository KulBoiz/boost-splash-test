    ...
    const _renderItem = ({ item }) => {
      ...
      return (
        <Pressable onPress={onPress} style={{
          ...
          {SurveyResultBase(item?.surveyDetails || [])?.splice(0, 5)?.map((el, index) => {
            return (<View
              key={index}
              style={{
                flexDirection: 'row',
      ...
      <View style={{ flex: 1, backgroundColor: color.palette.white }}>
        ...
        <View style={styles.container}>
          <View style={styles.total}>
            <AppText value={`Có ${surveyResults?.length} bộ hồ sơ đang chờ giải quyết!`} style={styles.title} />
